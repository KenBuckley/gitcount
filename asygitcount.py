...
def main(location,numusers):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        future_urs = {executor.submit(getReposByUsername, item["login"]):\
                     item for item  in jsonUsers["items"]}
        for future in concurrent.futures.as_completed(future_urs):
            user_repos_single = future.result()
            #build a dict with a count of merged pull requests per (user,Repo) key.
            with concurrent.futures.ThreadPoolExecutor(max_workers=NUM_WORKERS_INNER) as executor2:
                futures2 = {executor2.submit(getMergedPullRequests, \
                          username_repo[0], username_repo[1]): \
                          username_repo for username_repo in user_repos_single}
                for future2 in concurrent.futures.as_completed(futures2):
                    username_repo = futures2[future2]
                    user_repo_merged_count_dict[username_repo] = future2.result()     
    ...
    end_time = time.time()
    print("process time was:%sseconds" % (end_time - start_time))
    return output
